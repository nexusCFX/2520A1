...
static PyObject *Cal_freeFile(PyObject *self, PyObject *args) { return NULL; }
...
CalStatus readCalComp(FILE *const ics, CalComp **const pcomp) {
  ...
  while (!feof(ics)) {
    ...
    if (strcmp(prop->name, "BEGIN") == 0) {
      ...
      (*pcomp) = realloc(
          (*pcomp), (sizeof(**pcomp) + ((*pcomp)->ncomps) * sizeof(CalComp *)));
      ...
      if (eq == NULL || isspace(param[(int)(eq - param)])) {
        ...
        if (strchr(paramValues, ',')) { // Do multi value analysis
          ...
          for (int i = 0; i < length; i++) {
            ...
            if ((paramValues[i + 1] == ',' || paramValues[i + 1] == '\0') &&
                valueInQuotes == false) {
              ...
              if ((i + 2 <= strlen(paramValues)) && paramValues[i + 2] == '"') {
                valueInQuotes = (!valueInQuotes);
              }
          ...
          (newParam) =
              realloc((newParam), (sizeof(*newParam) + sizeof(char *)));
          assert(newParam != NULL);
    ...
    while (traverseProps) {
      ...
      if (traverseProps->param) {
        ...
        while (traverseParams) {
          ...
          strcat(output, traverseParams->value[traverseParams->nvalues - 1]);
